import {getPSCGroupsPrefixes} from "src/helpers/customerHelper";
...
// fetch customerCount
export const fetchCustomerCount = async (filter: string | null): Promise<number> => {
...
export const fetchCustomerPostcodes = async (limit: number, filter: string | null): Promise<string[]> => {
    const res = await fetch(`${DEFAULT_URL}(psc%20begins%20'${filter}').json?detail=custom:psc&limit=${limit}`);
    if (!res.ok) throw new Error("Failed to fetch post codes");
    const data: Data = await res.json();
    // getPSCGroupsPrefixes
    let result: string[] = []
    if (data?.winstrom?.adresar) {
        result = getPSCGroupsPrefixes(data.winstrom.adresar, filter ?? '')
    }
    return result
};